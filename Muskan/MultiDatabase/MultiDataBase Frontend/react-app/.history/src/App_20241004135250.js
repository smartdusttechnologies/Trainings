...
const App = () => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://localhost:7214/api/Employee/${id}`)
      .then((response) => {
        // Assuming you have state setter functions from useState hooks
        setName(response.data.name); // Update with actual state setter
        setAddress(response.data.address); // Update with actual state setter
        setSalary(response.data.salary); // Update with actual state setter
        // Any other state updates you need
      })
      .catch((error) => {
