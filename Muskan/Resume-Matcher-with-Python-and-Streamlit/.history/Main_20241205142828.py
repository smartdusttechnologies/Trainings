...
def save_resumes_to_folders(df_results, upload_files):
    base_directory = "classified_resumes"
    
    # Create the base directory if it doesn't exist
    if not os.path.exists(base_directory):
        os.makedirs(base_directory)
    
    # Loop over the dataframe results and save each file to its corresponding category folder
    for idx, row in df_results.iterrows():
        ...
        # Save the resume file in the corresponding folder
        file = upload_files[idx]
        save_path = os.path.join(category_folder, file.name)
        
        # Write the file to the corresponding category folder
        with open(save_path, "wb") as f:
            f.write(file.getbuffer())
        
        # Optionally, you can print/log the saved file path for verification
