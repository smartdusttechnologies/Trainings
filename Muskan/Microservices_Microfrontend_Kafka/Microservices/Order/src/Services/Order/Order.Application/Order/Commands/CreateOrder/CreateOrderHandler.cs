...
namespace Ordering.Application.Order.Commands.CreateOrder
{
     ...
     public class CreateOrderHandler(IApplicationDbContext context, ILoggingService<CreateOrderHandler> logger) : ICommandHandler<CreateOrderCommand, CreateOrderResult>
     {
          // Method to handle the CreateOrderCommand
          public async Task<CreateOrderResult> Handle(CreateOrderCommand command, CancellationToken cancellationToken)
          {
               await logger.LogInformationAsync($"Create order handler is hit {command}");
