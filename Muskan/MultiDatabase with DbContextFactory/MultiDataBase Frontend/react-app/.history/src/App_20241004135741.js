...
const App = () => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://localhost:7214/api/Employee/${id}`) // Send DELETE request
      .then((response) => {
        setName(response.data.name); // Update name state
        setAddress(response.data.address); // Update address state
        setSalary(response.data.salary); // Update salary state
      })
      .catch((error) => {
        console.error("There was an error deleting the employee!", error); // Log errors
      });
  };
