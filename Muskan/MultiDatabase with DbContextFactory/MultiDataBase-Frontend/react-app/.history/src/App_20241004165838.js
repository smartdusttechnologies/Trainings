...
const App = () => {
  ...
  useEffect(() => {
    const formEditButton = document.getElementById("form-edit");
    if (formEditButton) {
      formEditButton.addEventListener("click", (event) => {
        // Assuming `id` is passed from somewhere (e.g., through a data attribute)
    ...
    return () => {
      ...
      if (formEditButton) {
        formEditButton.removeEventListener("click", handleEdit);
      }
    };
  }, []);
