...
const Edit = ({ handleUpdate, editData, setEditData, editId, data }) => {
  const userToEdit = data.find((user) => user.id === editId); // Find the user to edit

  if (!userToEdit) return null; // If no user found, render nothing
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        handleUpdate(editId); // Call handleUpdate with the editId
      }}
    >
      <h2>Edit User</h2>
      <label>
