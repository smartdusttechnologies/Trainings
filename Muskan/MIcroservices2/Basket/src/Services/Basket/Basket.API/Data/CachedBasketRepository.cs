...
namespace Basket.API.Data
{
    ...
    public class CachedBasketRepository(IBasketRepository repository, IDistributedCache cache) : IBasketRepository
    {
        ...
        {
            // Update the basket from the database
            await repository.UpdateBasket(cart, cancellationToken);
            // Update the basket from the cache
            var serializedCart = JsonSerializer.Serialize(cart);
            await cache.SetStringAsync(cart.UserName, serializedCart, cancellationToken);
            // return true
            return true;
